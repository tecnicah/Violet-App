...
export class LegalReviewPage implements OnInit {
  ...
  //SORT REMINDER//
  sortData() {
    return this.renewal.reminderLegalReviews.sort((a, b) => {
  ...
  async addDocument() {
    ...
    modal.onDidDismiss().then((data: any) => {
      ...
      if (data.data.success) {
        data.data.legalReviewId = this.renewal.id;
        data.data.comment = "";
  ...
  async deleteDocument(id, i) {
    ...
    modal.onDidDismiss().then((data) => {
      ...
      if (data.data) {
        if (id != 0) {
          this._services.service_general_delete('ImmigrationServices/DeleteDocumentLR?id=' + id).subscribe(r => {
            if (r.success) {
  ...
  async deleteReminder(id, i) {
    ...
    modal.onDidDismiss().then((data) => {
      ...
      if (data.data) {
        if (id != 0) {
          this._services.service_general_delete('ImmigrationServices/DeleteReminderLR?id=' + id).subscribe(r => {
            if (r.success) {
              this.general_messages({
                title: "Success",
          ...
          this.renewal.reminderLegalReviews.splice(i, 1);
        }
      }
    })
    this.modalController.dismiss();
  ...
  save() {
    ...
    let data_comment_aux = this.renewal.commentLegalReviews;
    ...
    this._services.service_general_put("ImmigrationServices/PutLegalReview", this.renewal).subscribe((data => {
      if (data.success) {
        console.log(data);
        this.general_messages({
          title: "Success",
