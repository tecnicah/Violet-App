...
export class PredecisionOrientatonPage implements OnInit {
  ...
  public area: any = {
    ...
    extensionPredecisionOrientations: []
  };
  public data: any = {};
  public ca_visaType = [];
  ngOnInit() {
  ...
  //SORT REMINDER//
  sortData() {
    return this.area.reminderPredecisionOrientations.sort((a, b) => {
  ...
  async addDocument() {
    ...
    modal.onDidDismiss().then((data: any) => {
      ...
      if (data.data.success) {
        data.data.predecisionOrientationId = this.area.id;
        data.data.comment = "";
  ...
  async deleteDocument(id, i) {
    ...
    modal.onDidDismiss().then((data) => {
      ...
      if (data.data) {
        if (id != 0) {
          this._services.service_general_delete('RelocationServices/DeleteDocumentPDO?id=' + id).subscribe(r => {
            if (r.success) {
  ...
  async deleteReminder(id, i) {
    ...
    modal.onDidDismiss().then((data) => {
      ...
      if (data.data) {
        if (id != 0) {
          this._services.service_general_delete('RelocationServices/DeleteReminderPDO?id=' + id).subscribe(r => {
            if (r.success) {
              this.general_messages({
                title: "Success",
          ...
          this.area.predecisionOrientationId.splice(i, 1);
        }
      }
    })
    this.modalController.dismiss();
  ...
  save() {
    ...
    let data_comment_aux = this.area.commentPredecisionOrientations;
    ...
    this._services.service_general_put("RelocationServices/PutPreDecisionOrientation", this.area).subscribe((data => {
      if (data.success) {
        console.log(data);
        this.general_messages({
          title: "Success",
