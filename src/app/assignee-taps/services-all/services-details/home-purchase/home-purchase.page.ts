...
@Component({
  ...
  templateUrl: './home-purchase.page.html',
  styleUrls: ['./home-purchase.page.scss'],
})
export class HomePurchasePage implements OnInit {
  ...
  public area: any = {
    commentHomePurchases: [],
    reminderHomePurchases: [],
    documentHomePurchases: []
  };
  public data: any = {};
  public ca_visaType = [];
  ngOnInit() {
    ...
    this._services.service_general_get(`RelocationServices/GetHomePurchaseById?id=${this.data.service[0].id}`)
      .subscribe((response: any) => {
        ...
        if (response.success) {
          ...
          for(let i=0; i<this.area.commentHomePurchases.length; i++){
            this.area.commentHomePurchases[i].role = this.area.commentHomePurchases[i].createdByNavigation.role.role;
            this.area.commentHomePurchases[i].name = this.area.commentHomePurchases[i].createdByNavigation.name;
            this.area.commentHomePurchases[i].avatar = this.area.commentHomePurchases[i].createdByNavigation.avatar;
          }
  ...
  //SORT REMINDER//
  sortData() {
    return this.area.reminderHomePurchases.sort((a, b) => {
  ...
  public ca_estatus = [];
  public ca_currency = [];
  public catalog_statusPurchase = [];
  public caPropertyComission = [];
  public caComissionCurrency = [];
  async getCatalogos() {
  ...
  public services_catalogue = [];
  get_supplierPartner() {
    this._services.service_general_get(`SupplierPartnerProfile/GetSupplierPartnerServiceByServices?workOrderService=${this.area.workOrderServices}&supplierType=33&serviceLine=2`).subscribe((data => {
      console.log(data);
      if (data.success) {
  ...
  //***************************************************************************//
  public catalog_realtor = [];
  getRealtor() {
    if (this.area.supplierPartnerId != 0 && this.area.supplierPartnerId != '' && this.area.supplierPartnerId != null && this.area.supplierPartnerId != undefined) {
      console.log('supplier', this.area.supplierPartnerId);
      this._services.service_general_get(`SupplierPartnerProfile/GetConsultantContactsService?supplierPartner=${this.area.supplierPartnerId}&supplierType=33`).subscribe(resp => {
        if (resp.success) {
  ...
  async addDocument() {
    ...
    modal.onDidDismiss().then((data: any) => {
      ...
      if (data.data.success) {
        data.data.homePurchase = this.area.id;
        data.data.comment = "";
  ...
  async deleteDocument(id, i) {
    ...
    modal.onDidDismiss().then((data) => {
      ...
      if (data.data) {
        if (id != 0) {
          this._services.service_general_delete('RelocationServices/DeleteDocumentHomePurchase?id=' + id).subscribe(r => {
            if (r.success) {
  ...
  //FUNCION PARA AGREGAR REPLICA//
  addReminder() {
    this.area.reminderHomePurchases.push({
      "id": 0,
      "homePurchase": this.area.id,
  ...
  async deleteReminder(id, i) {
    ...
    modal.onDidDismiss().then((data) => {
      ...
      if (data.data) {
        if (id != 0) {
          this._services.service_general_delete('RelocationServices/DeleteReminderHomePurchase?id=' + id).subscribe(r => {
            if (r.success) {
              this.general_messages({
                title: "Success",
          ...
          this.area.reminderHomePurchases.splice(i, 1);
        }
      }
    })
    this.modalController.dismiss();
  ...
  save() {
    ...
    this.area.documentHomePurchases = this.temporalDocument;
    ...
    this.area.commentHomePurchases = [];
    this.area.commentHomePurchases = this.temporalComment;
    let data_comment_aux = this.area.commentHomePurchases;
    this.area.commentHomePurchases = [];
    ...
    for (let i = 0; i < data_comment_aux.length; i++) {
      if (data_comment_aux[i].comment != null && data_comment_aux[i].comment != undefined && data_comment_aux[i].comment.trim() != '') {
        this.area.commentHomePurchases.push(data_comment_aux[i]);
      }
    }

    console.log(this.area);
    this._services.service_general_put("RelocationServices/PutHomePurchase", this.area).subscribe((data => {
      if (data.success) {
        console.log(data);
        this.general_messages({
          title: "Success",
          body: "Home Purchase was updated",
          success: true
        });
        this.loader.loadingDismiss();
      ...
      for (let i = 0; i < this.coordinatorData.length; i++) {
        this._services.service_general_get('Catalog/GetUser/' + this.coordinatorData[i].coordinatorId)
          .subscribe((response: any) => {
            ...
            console.log("Estos son los coordinadores: ", this.coordinador);
          })
      }
    }
  }
  ...
  //**********************************************************************//
  public show_visit:boolean = false;
  showProperty(e){
    console.log(e);
    if(e.detail.checked){
