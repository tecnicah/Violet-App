...
@Component({
  ...
  templateUrl: './home-sale.page.html',
  styleUrls: ['./home-sale.page.scss'],
})
export class HomeSalePage implements OnInit {
  ...
  public addVisit_:boolean = false;
  ...
  ngOnInit() {
    ...
    this._services.service_general_get(`RelocationServices/GetHomeSaleById?id=${this.data.service[0].id}`)
      .subscribe((response: any) => {
        ...
        if (response.success) {
          ...
          for(let i=0; i<this.area.commentHomeSales.length; i++){
            this.area.commentHomeSales[i].role = this.area.commentHomeSales[i].createdByNavigation.role.role;
            this.area.commentHomeSales[i].name = this.area.commentHomeSales[i].createdByNavigation.name;
            this.area.commentHomeSales[i].avatar = this.area.commentHomeSales[i].createdByNavigation.avatar;
          }
          if(this.area.visitHomeSales.length > 0){
            this.show_visit = true;
            this.area.property = true;
          }
  ...
  //SORT REMINDER//
  sortData() {
    return this.area.reminderHomeSales.sort((a, b) => {
  ...
  async addDocument() {
    ...
    modal.onDidDismiss().then((data: any) => {
      ...
      if (data.data.success) {
        data.data.homeSale = this.area.id;
        data.data.comment = "";
  ...
  async deleteDocument(id, i) {
    ...
    modal.onDidDismiss().then((data) => {
      ...
      if (data.data) {
        if (id != 0) {
          this._services.service_general_delete('RelocationServices/DeleteDocumentHomeSale?id=' + id).subscribe(r => {
            if (r.success) {
  ...
  //FUNCION PARA AGREGAR REPLICA//
  addReminder() {
    this.area.reminderHomeSales.push({
      "id": 0,
      "homeSale": this.area.id,
  ...
  async deleteReminder(id, i) {
    ...
    modal.onDidDismiss().then((data) => {
      ...
      if (data.data) {
        if (id != 0) {
          this._services.service_general_delete('RelocationServices/DeleteReminderHomeSale?id=' + id).subscribe(r => {
            if (r.success) {
              this.general_messages({
                title: "Success",
  ...
  save() {
    ...
    this.area.documentHomeSales = this.temporalDocument;
    ...
    this.area.commentHomeSales = [];
    this.area.commentHomeSales = this.temporalComment;
    let data_comment_aux = this.area.commentHomeSales;
    this.area.commentHomeSales = [];
    ...
    for (let i = 0; i < data_comment_aux.length; i++) {
      if (data_comment_aux[i].comment != null && data_comment_aux[i].comment != undefined && data_comment_aux[i].comment.trim() != '') {
        this.area.commentHomeSales.push(data_comment_aux[i]);
      }
    }

    console.log(this.area);
    this._services.service_general_put("RelocationServices/PutHomeSale", this.area).subscribe((data => {
      if (data.success) {
        console.log(data);
        this.general_messages({
          title: "Success",
          body: "Home Sale was updated",
          success: true
        });
        this.loader.loadingDismiss();
  ...
  //***********************************************************************//
  addVisit(){
    this.area.visitHomeSales.push({
      "id": 0,
      "homeSale": this.area.id,
      "date": this.visit_data.date,
      "comment": this.visit_data.comment,
      "createdBy": this.userData.id,
      "createdDate": new Date(),
      "updateBy": this.userData.id,
      "updatedDate": new Date()
    })
    this.visit_data.date = null;
    this.visit_data.comment = '';
    this.addVisit_ = false;
  }
   //*****************************************************************//
   viewDoc(doc){
