...
@Component({
  selector: 'app-report-and-event-assignee',
  templateUrl: './report-and-event-assignee.page.html',
  styleUrls: ['./report-and-event-assignee.page.scss'],
})
export class ReportAndEventAssigneePage implements OnInit {
  ...
  public btn_show_edicion : boolean = false;
  ...
  save() {
    ...
    if (this.report.id != 0) {
      ...
      this.report.updateBy = this.userData.id;
      this.report.updatedDate = new Date();
      this.report.eventCloseDate = null;
      this.report.eventReportDate = null;
      this.report.quoteApproval = null;
      this.report.severityId = 1;
  ...
  statusAssignee(item){
    console.log(item);
    this.report.eventId = item.id;
    this.btn_show = false;
  }
  //*************************************************************************************//
  getSeverety(id){
    for (let i = 0; i < this.catalog_severity.length; i++) {
      if(this.catalog_severity[i].id == id){
         return this.catalog_severity[i].severity;
      }
    }
  }
  //*************************************************************************************//
  getStatus(id){
    for (let i = 0; i < this.status_event.length; i++) {
      if(this.status_event[i].id == id){
         return this.status_event[i].event;
      }
    }
  }
  //*************************************************************************************//
  async deletePhotoAssignee(item, i){
    const modal = await this.modalController.create({
    ...
    modal.onDidDismiss().then((data) => {
      ...
      if (data.data) {
        if (item.id != 0) {
          this._services.service_general_delete('RelocationServices/Photo/Assigned?id=' + item.id).subscribe(r => {
            if (r.success) {
              this.general_messages({
                title: "Success",
          ...
          this.report.assignedPhotos.splice(i, 1);
        }
      }
    })
    this.modalController.dismiss();
